...
function isApparelShape(shape) {
  return ["T-shirt", "Crewneck", "Hoodie"].includes(shape);
}

function buildPreviewVariantId(blueprintId, index) {
  return Number(`${blueprintId}${String(index + 1).padStart(4, "0")}`);
}

function getApparelPackage(product) {
  const pkg = product?.aiData?.apparelPackage;
  return pkg && typeof pkg === "object" ? pkg : null;
}

function buildPrintAreaPlaceholders({ position, concept, product, apparelPackage }) {
  const pkgPrompt =
    position === "back" ? apparelPackage?.backImagePrompt : apparelPackage?.frontImagePrompt;
  return [
    {
      position,
      images: [
        {
          // Placeholder — live integration replaces this with the uploaded Printify image id.
          id: position === "back" ? "IMG_BACK_PLACEHOLDER" : "IMG_FRONT_PLACEHOLDER",
          name: `${sanitizeCopy(concept.conceptName || product.title).slice(0, 52)} — ${position}`,
          type: "image/png",
          x: 0.5,
          y: position === "back" ? 0.48 : 0.45,
          scale: position === "back" ? 1 : 0.82,
          angle: 0,
          ...(pkgPrompt ? { promptSource: position } : {})
        }
      ]
    }
  ];
...
function buildPrintifyPreview(product, concept, podPrep, listingData, designPackage) {
  ...
  const apparelPackage = getApparelPackage(product);
  ...
  const variants = suggestedSizes.flatMap((size, sIdx) =>
    suggestedColors
      .filter((c) => c !== "—")
      .slice(0, 3)
      .map((color, cIdx, colorList) => ({
        // Numeric preview id. Live mode validates this again and should be
        // replaced with real Printify catalog variant ids when available.
        id: buildPreviewVariantId(blueprint.blueprint_id, sIdx * colorList.length + cIdx),
        price: priceCents,
  ...
  const apiPayloadPreview = {
    ...
    metadata: {
      __preview_only: true,
      __notes:
        "Generated by printifyPreviewService — not sent to Printify. Replace placeholder image ids and verify variant ids against the live Printify catalog before live publish.",
      sourceConceptId: concept.id,
      sourcePodPrepId: podPrep.id || null,
      sourceDesignPackageId: designPackage?.id || null,
      supportsBackPrint: isApparelShape(shape),
      includesBackPrint: printAreas.some((area) =>
  ...
  return {
    ...
    suggestedColors,
    suggestedSizes,
    printAreas: apparelPackage?.printAreas || null,
    apparelPackage,
